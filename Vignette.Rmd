...
source("MSFAST/Resample_Scores.R")
```
...
```{r form_inputs}
# Reset participant ids to sequential integers
...
# Get domain and translate to (0, 1)
domain = S_df$Arg
minv = min(domain)
maxv = max(domain)
scaled_domain = (domain - minv) / (maxv - minv)
fit_df = fit_df %>%
  mutate(Arg = (Arg - minv)/(maxv - minv))
...
# Function to form the requisite list using the default Orthogonalized B-Spline basis
...
fit_joint = stan("MSFAST/MSFAST.stan", 
                 data = data_list, 
                 chains = 4, 
                 cores = 4, 
                 warmup = 2000, 
...
Note that the above block does not leverage parallelization over covariates through multithreading. To leverage this computational speed-up (after ensuring sufficient available resources), one can use the following.

```{r fit_parallel, eval=FALSE}
# Enable STAN threading, specify threads per core
Sys.setenv(STAN_NUM_THREADS=2) 

# Fit the model with threading enabled
fit_joint = stan(file = "FAST/MSFAST_Parallel.stan",
                 data = data_list,
                 chains = 4,
                 cores = 4,
                 warmup = 2000,
...
We first extract posterior estimates of MSFAST to be used as our fixed point $\widetilde{\boldsymbol{\Phi}}$ for the Procrustes-based alignment of posterior FPC samples described in the manuscript. We also fit the multivariate FACEs to use as a comparison.
...
# Call to mFACEs function
face_fit = mface.sparse(face_data, argvals.new = domain, knots = Q-3,
                        knots.option = "quantile", pve = 0.999)
```

We next check the Gelman-Rubin statistics for all relevant model components (after alignment). This is done automatically using the "RHat_FAST()" function as follows. This function calculates the median and max RHat observed for each FPC/covariate grouping within parameter families. 

```{r rhat_eval}
# Calculate estimates first, for target of alignment
objects = FAST_extract(fit_joint, data_list$B, data_list)
EF_ests = Latent_RSV(objects$Weights, objects$Score, data_list$B, P,
                     domain)
phi_tilde = EF_ests %>%
  pivot_wider(names_from = FPC_Num, values_from = FPC_Val) %>%
  arrange(Var, Arg) %>%
  select(-c(Arg, Var)) %>%
  as.matrix()
...
# Extract FPC estimates
EF_ests = Latent_RSV(objects$Weights, objects$Score, basis_mat, P,
                     domain)
phi_tilde = EF_ests %>%
  pivot_wider(names_from = FPC_Num, values_from = FPC_Val) %>%
  arrange(Var, Arg) %>%
...
{
  FACE_FPC = data.frame(face_fit$eigenfunctions[,1:K])
...
We begin by forming the latent trajectory predictions using data from the two held-out participants. We form predictions using up to 150, 300, and 450 days of data, where we predict the latent trajectory out to 500 days.

```{r dynamic_prediction_calculation, message=FALSE}
...
trajectories = map(thresholds, function(age_thresh) { # Predictions at each threshold
  sub_traj = map(for_prediction, function(sid) { # Predictions for each participant
    ...
    Bi = list(
      length = basis_mat[subset %>% filter(Metric == "zlen") %>% pull(S), ], 
    ...
    mutate(Var = case_when(Metric == "zlen" ~ 1, # Rescale the data
                           TRUE ~ 2)) %>%
    left_join(data_list$consts, by = "Var") %>%
    mutate(Traj = Traj * sd_Y + mu_Y) %>%
    select(-c(Var)) %>%
    mutate(Metric = case_when(Metric == "zlen" ~ "Length",
                              TRUE ~ "Weight"))
  
  return(
    sub_traj %>% # Summarize trajectories by pointwise mean and credible intervals
